...
For this workshop, it is strongly recommended that groups install and use the desktop version of the Arduino IDE (version 2.3.10), which is available for Windows (versions 10 and 11), Linux, and macOS. Chromebook users can attempt to install Arduino Cloud via the [Play store](https://www.arduino.cc/en/software/#:~:text=Arduino%20Cloud%20on%20Chromebook); however, this interface differs from the desktop version and may make it more difficult to follow along. 
